...
export default function EquityCurveChart() {
  ...
  const renderState = () => {
    if (runStatus === 'loading') {
      return <div className="mt-6 text-center text-sm text-slate-400">Running backtest…</div>;
    }
    if (runStatus === 'error') {
      return (
        <div className="mt-6 text-center text-sm text-danger">
          {runError ?? 'Failed to execute backtest'}
        </div>
      );
    }
    if (!result) {
      return <div className="mt-6 text-center text-sm text-slate-500">Select a strategy to view results.</div>;
    }
    return null;
  };
